...
public class StopReasonGroupController {
	...
	@PostMapping("{id}")
	public ResponseEntity<?> update(@PathVariable long id, @Valid @RequestBody StopReasonGroupUpdateBody body) {
		return new ApiAnwser<>(stopReasonGroupService.update(id, body)).toResponseEntity();
	}
	
	@DeleteMapping("{id}")
	public ResponseEntity<?> delete(@PathVariable long id) {
		return new ApiAnwser<>(stopReasonGroupService.delete(id)).toResponseEntity();
	}
