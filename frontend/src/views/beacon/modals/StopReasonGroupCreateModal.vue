<template>
	<creation-modal ref="stopReasonGroupCreateModalModal" :title="title" :edit="editId !== null" :endpoint="endpoint" :payload="inputs" :on-open="refresh" :on-close="resetInputs" @loaded="payloadToInputs" @created="(object) => $emit('created', object)" @updated="(object) => $emit('updated', object)">
		<template v-slot="{ loading }">
			<v-list>
				<v-list-item>
					<v-text-field v-model="inputs.name" :label="$t('common.fields.name')" :disabled="loading" required></v-text-field>
				</v-list-item>
				<v-list-item>
					<v-list-item-content>
						<v-select v-model="inputs.childrenIds" :label="$t('common.fields.children')" multiple :items="items.stopReasons" item-text="name" item-value="id" :loading="dependencyLoading"></v-select>
					</v-list-item-content>
					<v-list-item-avatar>
